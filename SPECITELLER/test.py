...
# _,xst= getFeatures(os.path.join(params.nlipath,'vln.txt'))
_,xst= getFeatures('dataset/data/vln.txt')
_,xsu = getFeatures('dataset/data/vln.txt')
...
for split in ['s1']:
    for data_type in ['train', 'valid', 'test', 'unlab', 'trainu']:
        eval(data_type)[split] = np.array(
            [['<s>'] + [word for word in sent.split() if word in word_vec] for sent in eval(data_type)[split]],
            dtype=object
        )
...
# pdtb_net2 = torch.load('savedir/3os' + params.outputmodelname, weights_only=True)
print(pdtb_net)
